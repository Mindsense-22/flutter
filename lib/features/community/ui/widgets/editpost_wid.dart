...
class EditpostWid extends StatefulWidget {
  final FeedPost post;
  const EditpostWid({super.key, required this.post});

  @override
  State<EditpostWid> createState() => _EditpostWidState();
}

class _EditpostWidState extends State<EditpostWid> {
  ...
  void initState() {
    super.initState();
    _intention = _matchIntention(widget.post.type);
    _visibility = _matchVisibility(widget.post.visibility);
    _audience = _matchAudience(widget.post);
  ...
  String _matchIntention(String type) {
    final normalized = type.toLowerCase();
    return _intentions.firstWhere(
      (item) => item.toLowerCase() == normalized,
      orElse: () => _intentions.first,
    );
  }

  String _matchVisibility(String visibility) {
    final normalized = visibility.toLowerCase();
    return _visibilities.firstWhere(
      (item) => item.toLowerCase() == normalized,
      orElse: () => 'Nickname',
    );
  }

  String _matchAudience(FeedPost post) {
    if (post.circle == null) return 'Global';

    String? circleName;
    if (post.circle is Map) {
      circleName = (post.circle as Map)['name'] as String?;
    }

    if (circleName != null && _audiences.contains(circleName)) {
      return circleName;
    }
    return 'Global';
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 4.h),
          Text(
            'Update your reflection, progress, or encouragement.',
            style: TextStyle(
              fontSize: 12.sp,
              color: Colors.grey[400],
            ),
              ...
              Consumer<CommunityProvider>(
                builder: (context, provider, child) {
                  return MaterialButton(
                    onPressed: provider.postisLoading
                        ? null
                        : () async {
                            ...
                            if (success && context.mounted) {
                              ...
                              customSnackbar(context, false, 'Post updated');
